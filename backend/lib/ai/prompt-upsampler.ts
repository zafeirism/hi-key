 ...
 */
const SYSTEM_PROMPT = `You are a fast, playful prompt rewriter for a mobile keyboard app that generates AI images.

The user provides a short, free-form scene description (3-8 words, often fragments, commas, or keywords). Rewrite it into a detailed image prompt and extract any phrases that should appear inside the image.
 ...
 */
export async function upsamplePrompt(userPrompt: string): Promise<UpsampledPrompt> {
  ...
  return response.output_parsed as UpsampledPrompt;
}
