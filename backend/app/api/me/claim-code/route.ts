import { NextResponse } from 'next/server';
import { withAuth } from '@/lib/auth/jwt';
import { toDisplayCredits } from '@/lib/credits/balance';
import {
  AlreadyClaimedError,
  AlreadyDoubledError,
  CodeNotFoundError,
  InvalidCodeError,
  claimWaitlistCode,
} from '@/lib/waitlist/service';

export const POST = withAuth(async (request, user) => {
  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: 'invalid_json' }, { status: 400 });
  }

  const code = (body as { code?: unknown })?.code;
  if (typeof code !== 'string') {
    return NextResponse.json({ error: 'code is required' }, { status: 400 });
  }

  try {
    const balance = await claimWaitlistCode(user.id.toLowerCase(), code);
    return NextResponse.json({
      credits: {
        sub_credits: toDisplayCredits(balance.sub_credits_mills),
        extra_credits: toDisplayCredits(balance.extra_credits_mills),
        next_reset_at: null,
      },
    });
  } catch (err) {
    if (err instanceof InvalidCodeError) {
      return NextResponse.json({ error: 'invalid_code' }, { status: 400 });
    }
    if (err instanceof CodeNotFoundError) {
      return NextResponse.json({ error: 'code_not_found' }, { status: 404 });
    }
    if (err instanceof AlreadyClaimedError) {
      return NextResponse.json({ error: 'already_claimed' }, { status: 409 });
    }
    if (err instanceof AlreadyDoubledError) {
      return NextResponse.json(
        {
          error: 'already_doubled',
          credits: {
            sub_credits: toDisplayCredits(err.balance.sub_credits_mills),
            extra_credits: toDisplayCredits(err.balance.extra_credits_mills),
            next_reset_at: null,
          },
        },
        { status: 409 }
      );
    }
    console.error(
      `${new Date().toISOString()} Failed to claim waitlist code: ${
        err instanceof Error ? err.message : String(err)
      }`
    );
    return NextResponse.json({ error: 'internal_error' }, { status: 500 });
  }
});
